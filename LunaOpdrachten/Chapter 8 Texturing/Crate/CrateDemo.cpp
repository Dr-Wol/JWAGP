...
private:
	...
	ID3D11ShaderResourceView* mOpplakTexSRV;
...
void CrateApp::DrawScene()
{
	...
	ID3DX11EffectTechnique* TexTech = Effects::BasicFX->Light2TexTech;
...
void CrateApp::BuildGeometryBuffers()
{
	...
	geoGen.CreateBox(1.0f, 0.1f, 2.0f, box);
...
void CrateApp::BuildTextureBuffers()
{
	GeometryGenerator::MeshData grid;

	GeometryGenerator geoGen;

	geoGen.CreateGrid(0.9f, 1.5f, 50, 50, grid);

	mTexGridCount = grid.Indices.size();
...
/*	std::vector<UINT> indices(3 * mWaves.TriangleCount()); // 3 indices per face
	...
	D3D11_BUFFER_DESC ibd;
	ibd.Usage = D3D11_USAGE_IMMUTABLE;
	ibd.ByteWidth = sizeof(UINT)* mTexGridCount;
	ibd.BindFlags = D3D11_BIND_INDEX_BUFFER;
	ibd.CPUAccessFlags = 0;
	ibd.MiscFlags = 0;
	D3D11_SUBRESOURCE_DATA iinitData;
