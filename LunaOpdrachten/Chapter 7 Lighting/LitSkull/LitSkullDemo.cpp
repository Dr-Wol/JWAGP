...
public:
	...
	void DrawScene(); 
    void DrawSkulls(CXMMATRIX a_ViewProjection);
...
void LitSkullApp::DrawSkulls(CXMMATRIX a_ViewProjection)
{
    auto activeTech = Effects::BasicFX->Light1Tech;
	...
	for (int k = 0; k < n; ++k)
	{
		for (int i = 0; i < n; ++i)
		{
			for (int j = 0; j < n; ++j)
			{
				...
				world = XMMatrixMultiply(skullScale, world);

				world *= XMMatrixTranslation(-4+ k * 2.0f, -4+ i*2.0f, -4+ j*2.0f);
					
					
				XMMATRIX worldInvTranspose = MathHelper::InverseTranspose(world);


				XMMATRIX worldViewProj = world*a_ViewProjection;
